...
import numpy as np
import math
img = cv2.imread('D:/learn/train_2024/train1/testpic/8.jpg')
...
# s_contour_sort=sorted(contour,key=cv2.contourArea,reverse=True)[0]
# cv2.drawContours(img_s,contour,0,(0,0,255),3)
# cv2.imshow('img_s',img_s)
# point=np.array(contour,np.int32)
# point=point.reshape((-1,1,2))
# s_area=cv2.contourArea(point)
# #print(area)
# # cv2.waitKey(0)
# # cv2.destroyAllWindows()
# # return area
...
cv2.drawContours(img_c,c_contour_sort,0,(0,0,255),3)
cv2.imshow('img_c',img_c)
points=np.array(contour,np.float32)
points=points.reshape((-1,1,2))
c_area=cv2.contourArea(points)
cv2.waitKey(0)
cv2.destroyAllWindows()
